...
*   The accessed server is not listed in the NTLM trusted uris section (Local Intranet Area).
...
::
    ...
    cAppMenuOptions["texts"] = {
            ...
            "sub-title":"Syspertec Virtel V4.64 Application Menu",
            };
...
::
    ...
    // Regular expression is used to filter out qualifying applications
    cAppMenuOptions["levels"][0] = {
    "title":"CICS Production Regions",
    "criteria":"tran",
    "regexp": /cicsp/i
    ...
    // Note that the Criteria and Regular expression properties are not used.
    cAppMenuOptions["levels"][6] = {
    "title":"Other Sessions",    
    };
...
::
    ...
    // ----------------------------------------
    // Remove the // comment mark on the next line to set this option:
    // cAppMenuOptions.close_VWA_when_disconnects = true;
    ...
    // Remove the // comment mark on the next line and replace "xxx" by the target pf key value, and "yyy" by the external name of target VIRTEL transaction to be accessed by pressing the pf key.
