...
1. After unzipping virtel464mvs.zip run job $ALOCDSU to create the TRANSFER.XMIT file.

2. Upload the virtel464mvs.xmit file to the TRANSFER.XMIT file IN BINARY MODE.
...
4. Apply the PTFs in the allptfs-mvs464.txt file using job ZAPJCL in the VIRTEL CNTL library. If allptfs-mvs464.txt doesn't exist skip this step.

5. Use the SETPROG APF command to add the VIRTEL LOADLIB to your system APF authorized program library list::

    SETPROG APF,ADD,DSN=yourqual.VIRT464.LOADLIB,VOL=volser
...
15. Apply any "update" maintenance (virtel464updtnnnn.zip) according to the instructions in the Readme-updtnnnn.txt file in the virtel464updtnnnn.zip if available. Skip this step if no zip file is available.
