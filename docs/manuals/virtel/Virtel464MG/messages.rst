...
.. 
    ...
    LOCK=00000,LPKALIVE=00000,USERFUNC=Y,NFORCEIP=N,CRYPTS=NONE
...
Module
    VIRHTTP
Meaning
    Virtel is running with the option HDRSEC=(,,SESSCOOKIE) in the TCT, and a request using an existing VirtelSession code was blocked by Virtel because the supplied Session Cookie was invalid or missing.
Action
    The request is blocked by Virtel. The security team should be notified of this error to verify if the user at the specified ip address is actually trying to steal someone else's session.
