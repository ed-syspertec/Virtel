...
Virtel provides three models. Web access, Web modernisation and Web integration.
...
With modernisation, for example, several 3270 round trips can be made to the host application within a presentation scenario. Data could be extracted from the indivdual host responses and encapsulated into one HTML page. For example when a user requests a "list" this could result in a sequence of key strokes and displays. Taking the example:-
...
pathname
    Represents a VIRTEL directory containing HTML pages or other elements. Refer to the section :ref:`“How the path name corresponds to a VIRTEL directory” <#_V457UG_How_the_path_name>` below for details of how the pathname is used to select the VIRTEL directory.
...
pagename
    ...
    the directory definition (see :ref:`“Directory Management” <#_V457UG_directory_management>`).
...
sessionid
    ...
    *{{{SESSION-CODE}}}* tag embedded in the template page (see :ref:`“Session And Context Management” <#_V457UG_session_context_mangement>`).
...
pagename
    Is the name of the template page to be used for displaying the print data. This page must contain the special *{{{PRINT}}}* tag (see :ref:`“3287 Printing” <#_V457UG_printing>`) at the point at which the print data are to be inserted in the page.
...
The VirtelSession cookie should be generated by Javascript as shown in the example below. This function may be added to the custom Javascript script (see :ref:`“Site customization of Javascript functions” <#_V457UG_customization_Javascript_functions>`):
...
capability
    ...
    generated by the :ref:`“{{{SET-HEADER}}}” <#_V457UG_SET-HEADER>` tag.
...
A security code, which identifies a user during a HTTP page upload request, is normally transmitted between the browser and VIRTEL as a cookie (see :ref:`“Uploading HTML Pages” <#_V457UG_uploading_HTML_pages>`). If, for security or other reasons, your browser does not allow the use of cookies, then the security code can, as an alternative, be embedded in a URL of the form shown below:-
...
A URL embedded in a page sent by VIRTEL may contain a VirtelUserSignon parameter in order to propagate the user’s signon credentials to another page. The value of the parameter is generated by the :ref:`"({{{USER-SIGNON-CODE}}} tag "<#_V457UG_user-signon-code>`. This can be useful in cases where the browser does not propagate the user name and password from one page to another, for example when using VIRTEL transaction security type 4. The format of a URL containing a signon code is shown below:-
...
.. _#_V457UG_directory_management:
...
HTML and XML template pages and other entities such as CSS stylesheets, JavaScript procedures, and GIF or JPEG images used by VIRTEL Web Access are stored in directories within a VSAM KSDS managed by VIRTEL. Each KSDS defined to VIRTEL can contain one or more directories. The system administrator can upload pages and other elements into a VIRTEL directory by using a web browser or by e-mail, as described in detail in the section :ref:`“Uploading HTML Pages” <#_V457UG_uploading_HTML_pages>`.
...
.. _#_V457UG_session_context_mangement:
...
The SESSION-CODE tag allows a session code to be inserted in the template page in the format VirtelSession=xxxxxxxxxxxxxxxx. The value of the session code changes after each message. For examples of its use, see :ref:`“Transmitting Data To The Host” <#_V457UG_transmitting_data>`, and :ref:`“3287 Printing” <#_V457UG_printing>`.
...
.. index::
   pair: ACTION$ REFRESH-TERMINAL; tags
...
name
    Form name. The name of the form is used in JavaScript procedures to refer to various elements of the form. For an example, see the section :ref:`“Cursor management” <#_V457UG_cursor_management>`.
...
An alternative form of the GENERATE-HTML tag allows the position and length to be specified by means of a symbolic name defined in a previous DEFINE-DFHMDF-NAME or DEFINE-DFHMDF-COLUMN tag (see :ref:`“{{{DEFINE-DFHMDF-COLUMN}}}” <#_V457UG_define_dfhmdf_column>`.
...
name
    ...
    Normally, any binary zeroes in the copied data will be removed. However, if the BLANK-BINARY-ZEROES option is set (see :ref:`“Setting and unsetting local options” <#_V457UG_setting_local_options>`) then binary zeroes will be converted to blanks.

.. _#_V457UG_create-variable-from:
...
.. _#_V457UG_define_dfhmdf_column:
...
For example, the tag {{{ GENERATE-VARIABLES (01,01,1920) "MYPREFIX" }}} generates a set of table variables describing the entire contents of the 3270 screen. Normally, any binary zeroes in the field will be removed from the VALUE. However, if the BLANK-BINARY-ZEROES option is set (see :ref:`“Setting and unsetting local options” <#_V457UG_setting_local_options>`) then binary zeroes will be converted to blanks.
...
For certain fields, the action of the ADD-TO-FIELDS tag may be nullified by the NO-ADD-TO-CHECKBOX and NO-ADDTO-LISTBOX local options (see :ref:`“Setting and unsetting local options” <#_V457UG_setting_local_options>`).

.. note:: The ADD-TO-FIELDS tag affects input fields only   

.. _#_V457UG_cursor_management:
...
.. _#_V457UG_setting_local_options:
...
BLANK-BINARY-ZEROES
    Affects the processing of the COPY-FROM and GENERATE-VARIABLES tags (see :ref:`“Inserting host application data in a page” <#_V457UG_inserting_host_data>`).
...
HTML-ESCAPES, JAVASCRIPT-ESCAPES, JSON-ESCAPES, NO-ESCAPES, XML-ESCAPES
    Affects the processing of the CURRENT-VALUE-OF, TRIMMED-VALUE-OF and NO-BLANKS-VALUE-OF tags (see :ref:`“Handling table variables” <#_V457UG_table_variables>`). Similarly affects the processing of the values generated by GENERATEVARIABLES.
ID
...
A table variable is a list of values sent to VIRTEL by a host application in a structured field of type “FAE5 or FAE6”,^page 247. Table variables may also be created by means of a VIRTEL tag embedded in the page template (see :ref:`“CREATE-VARIABLE-FROM” <#_V457UG_create-variable-from>`), via a scenario (see :ref:`“COPY$ instructions” <#_V457UG_copy$>`), or by means of a S VARIABLE command contained in the host 3270 data stream (see :ref:`“HOST4WEB commands” <#_V457UG_host4web>`).
...
The local options HTML-ESCAPES, JAVASCRIPT-ESCAPES, JSON-ESCAPES, NO-ESCAPES, and XML-ESCAPES (see :ref:`“Setting and unsetting local options” <#_V457UG_setting_local_options>`) may affect the processing of this tag.
...
The WHEN-NEXT-EVENT and END-WHEN-NEXT-EVENT tags allow an XML template to be written which uses variables generated by a commarea-to-output conversion scenario. These tags work in conjunction with the $EVENT$ variable generated by the “MAP$ EVENTUAL-AREA and :ref:`MAP$ ELSE-THEN-AREA” <#_V457UG_MAP$_EVENTUAL_ELSETHEN>`. The section of the page enclosed by the WHEN-NEXT-EVENT and END-WHEN-NEXT-EVENT tags is generated only if the current value of the $EVENT$ variable matches the specified eventname.
...
The WHILE-EVENT and END-WHILE-EVENT tags work in conjunction with the $EVENT$ variable generated by the :ref:`“MAP$ EVENTUAL-AREA and MAP$ ELSE-THEN-AREA” <#_V457UG_MAP$_EVENTUAL_ELSETHEN>`. The WHILE-EVENT tag marks the start of a loop. Everything between the WHILE-EVENT and END-WHILE-EVENT tags is generated once for each value of the $EVENT$ variable, as long as the current value of the $EVENT$ variable matches the specified eventname
.

.. _#_V457UG_debugging_create_variable:
...
The CREATE-VARIABLE-IF tag with the TRACING-SCENARIO parameter retrieves the contents of the scenario trace created by the :ref:`“Capability Tokens” <#_V457UG_capability_tokens>`). If the trace is active, the variable varname will be created. The value of this variable is a JSON structure (see example below) which can be used by Virtel Studio. 

The trace data for a specific terminal may be obtained by specifying an x-Virtel-Debug=capability-token parameter in the URL. The :ref:`“capability tokens” <#_V457UG_capability_tokens>` is generated by a SET-HEADER tag issued by the terminal which owns the trace data.
...
For more informations, see :ref:`“CREATE-VARIABLE-IF” <#_V457UG_create-variable-if>`.
...
APPLICATION-IS-CONNECTED
    ...
    For more informations, see :ref:`“CREATE-VARIABLE-IF” <#_V457UG_debugging_create_variable>`, in the “Debugging facilities” section.
...
When used in conjunction with a security type 4 transaction, the DECLARE-FIELD-AS tag allows VIRTEL to obtain the userid and password from fields embedded in the HTML page. For an example, see :ref:`“Signon using HTML fields” <#_V457UG_signon_using_HTML>`.
...
ENCRYPTED
    Indicates that VIRTEL expects the browser to encrypt the field using the session key specified elsewhere in the page (see :ref:`“DECLARE-FIELD-AS CRYPTO-SESSION-KEY” <#_V457UG_declare_field_crypto>`).
...
The USER-SIGNON-CODE tag generates a unique value called a signoncode which may be used with the VirtelUserSignon parameter of the URL to propagate the signed-on user’s credentials to another page (see :ref:`“Propagation of signon by URL” <#_V457UG_signon_propagation>`).
...
.. _#_V457UG_declare_field_crypto:
...
These files may then be passed to the VIRSVPDF service program which invokes MakePDF to generate output files in PDF format. Refer to :ref:`“PDF output generation” <#_V457UG_PDF>` for an example scenario.
...
.. _#_V457UG_correspondent_management:
...
- a correspondent may be authorized to upload HTML pages and other elements into an HTMLTRSF file. For further details, refer to :ref:`“Uploading pages by SMTP” <#_V457UG_http_uploading_pages_smtp>` and :ref:`“Uploading pages by HTTP (secured by cookie)” <#_V457UG_http_uploading_pages>`.
...
.. _#_V457UG_http_uploading_pages_smtp:    
...
1. Activate the e-mail correspondent: see “Account activation” under the heading :ref:`“Correspondent Management” <#_V457UG_correspondent_management>`. This triggers the sending of an e-mail containing the security code, as in the following example:-
...
.. _#_V457UG_http_uploading_pages:
...
1. (First time only) Activate the e-mail correspondent: see :ref:`“Account activation” <#_V457UG_account_activation>`. This triggers the sending of an e-mail containing the security code, as in the following example:
...
.. _#_V457UG_http_uploading_pages_signon:
...
uplbas
    external name of the upload transaction in VIRTEL which specifies the target directory (HTMLBAS). See :ref:`“Uploading pages by HTTP (secured by signon)” <#_V457UG_http_uploading_pages_signon>` for a list of upload transactions.
...
Display Style
    ...
    White
        ...
        .. note:: To add or remove a style, see :ref:`“Customize display style” <#_V457UG_customize_display>`). Administrators can add some new customs style as describe in the section :ref:`“How to customize display style” <#_V457UG_customize_display>`.
...
.. _#_V457UG_global_modifications:    
...
The w2hparm.js file as delivered is just a dummy file and contains no default settings. See `“Appendix A - w2hparm values” <#_V457UG_w2hparm_defaults>` for a list of the possible w2hparm values.
...
.. index::
   triple: SCS printers; LU Type 1; Printing
   triple: 3270 data stream; LU Type 3; Printing
...
.. _#_V457UG_accessing_VTAM_applications:   
...
.. _#_V457UG_Customization_Colors_Logo:   
...
12. Run Virtel in "Compatibility mode" by updating the TCT. See `"Running in Compatibility Mode"<#_V457UG_compatibility mode>` for further details.
...
You can also use a custom Javascript to remove icons individually from the toolbar, see :ref:`“Removing unwanted toolbar icons” <#_V457UG_remove_icons>`.
...
To hide the toolbar completely, see :ref:`“Hiding the toolbar” <#_V457UG_hide_toolbar>`.

To hide only the VIRTEL Application name, see :ref:`“Showing / Hiding server informations” <#_V457UG_hide_information>`.
...
As of July, 2017 Microsoft has yet to release an API to support Copy, Cut and Paste operations within the Edge browser and the clipboard. These opertions will not work in Virtel when using Edge.     
...
.. index::
   triple: Compatibility Option id; Customization; Transactions  
...
.. index::
   triple: Defining Transaction options; Customization; Transactions  
...
This section discusses the answer to the first concept, the second one is discussed in the section :ref:`“Web Modernisation VIRTEL Scenarios” <#_V457UG_virtel_scenarios>`.
...
.. index::
   ...
   triple: Using Virtel storage for macros; Virtel; Dynamic Directory Interface    
...
Indicates that the transaction is now a service transaction (see :ref:`“Service Transactions” <#_V457UG_service_transactions>`).
...
AND=(SCRIPT,'string')
    (optional) after sending the message to the application, VIRTEL executes the string as a Connection/Disconnection script (see :doc:`“Connection/Disconnection Scripts" <../Virtel457CN/connectivity_guide>`)  in the VIRTEL Connectivity Reference manual).
AND=(PROCESS-RESPONSE,'string')
    (optional) allows an input scenario to explicitly request that control is to be returned when the next message is received from the application. The string is optional. If specified, after sending the message to the application, VIRTEL executes the string as a Connection/Disconnection script (see :doc:`“Connection/Disconnection Scripts" <../Virtel457CN/connectivity_guide>` in the VIRTEL Connectivity Reference manual). 
...
MAXTIME=nnn
     (optional) specifies the maximum time, in hundredths of a second, that the scenario will wait for a message from the application. When this time expires, the APPLICATION-TIMEOUT flag will be set (see :ref:`“IF$ instruction” <#_V457UG_IF$>`) and the scenario will resume execution. If the MAXTIME parameter is not specified, then the scenario will wait indefinitely, or until VIRTEL disconnects the session when the timeout value of the entry point is exceeded.
LASTMSG=YES
...
.. index::
   pair: ACTION$ Examples; Scenarios
...
.. index::
   pair: CASE$ Instruction; Scenarios
...
CURRENT-SCREEN-POSITION
    The tested variable is located at the current screen position as set or incremented by :ref:`“SET$ SCREEN-POSITION” <#_V457UG_SET$_SCREEN-POSITION>`.
LTRIM
...
.. index::
   pair: COPY$ Instruction; Scenarios
...
name1
    the name of an HTTP query parameter (see :ref:`“VIRTEL URL formats” <#_V457UG_url_formats>`)
row,col,len
...
If the parameter name1 is not present in the input request, the NOT-FOUND condition will be raised. This condition can be tested by means of the :ref:`“IF$ instruction” <#_V457UG_IF$>`.
...
NEW-PASSTICKET-FOR-TRANSACTION
    Generate a PassTicket allowing the current USERID. If the generation fails, the NOT-FOUND condition will be raised. This condition can be tested by means of the :ref:`“IF$ instruction” <#_V457UG_IF$>`.
...
item
    When class is NAME-OF or VALUE-OF, the following VIRTEL data items may be specified:
        ...
        CURRENT-FOREACH-VARIABLE
            Value of the variable at current index in a FOREACH loop (see :ref:`“FOREACH$ VALUE-IN-VARIABLE” <#_V457UG_FOREACH_variable>`).
        ...
        CURRENT-HEIGHT
            The current values of the named field in the :ref:`“SET$ SCREEN-POSITION” <#_V457UG_SET$_SCREEN-POSITION>`.
        DATE-TIME
            The current date and time (14 characters in the format YYYYMMDDHHMMSS)
        DIRECTORY
            The current VIRTEL directory name
...
row,col,len
    row, column, and length of the destination field on the screen. If the SCREEN parameter is not specified, the variable will be copied to the current cursor position. For row, col and len, a value of '=' means the value of the current row, col or len as set by :ref:`“SET$ SCREEN-POSITION” <#_V457UG_SET$_SCREEN-POSITION>`.
type
    (optional) may indicate one of the following values:
...
class
    identifies the type of VIRTEL system field to be updated. The following values may be specified:
        ...
        OLD-PASSTICKET-FOR-TRANSACTION
            Evaluates the Passticket currently in 'varname' for the transaction 'item'. If the evaluation fails, the NOT-FOUND condition will be raised. This condition can be tested by means of the :ref:`“IF$ instruction” <#_V457UG_IF$>`.
item
    When class is NAME-OF or VALUE-OF, the following VIRTEL data items may be specified:
...
TRACE,SCENARIO or NOTRACE,SCENARIO
    Starts or stops a trace on the scenario. The trace is downloadable by means of the :ref:`“CREATE-VARIABLE-IF” <#_V457UG_create-variable-if>` tag intended for use by Virtel Studio.
SNAP
...
AS-PFKEY
    Indicates that the indicated portion of the screen is to be interpreted as a function key hyperlink. The value of the function key (ENTER, PF1, etc) can be specified by parameter P2. If P2 is not specified, then the data at the indicated screen position is interpreted as the name of the function key. The values allowable as function key names are the same as those for :ref:`“PfkField” <#_V457UG_pfkField>`)
AS-PARAMETER
    Indicates that the indicated portion of the screen is to be treated as a hyperlink which calls a JavaScript function. The data at the indicated screen position is passed as a parameter to the JavaScript function. The name of the JavaScript function is specified in the TO parameter of the $DECLARE instruction. See :ref:`“JavaScript functions” <#_V457UG_Javascript_functions>` for further details.
AS-HREF
...
.. _#_V457UG_Javascript_functions:

.. index::
   pair: DECLARE and JavaScript functions; Scenarios
...
.. index::
   pair: DECLARE Examples; Scenarios
...
.. index::
   pair: ENDFOR$ Instruction; Scenarios
...
.. index::
   pair: FILTER Instruction; Scenarios
...
.. index::
   pair: FOREACH Instruction; Scenarios
...
.. index::
   pair: GOTO$ Instruction; Scenarios
...
.. index::
   pair: HANDLE$ Instruction; Scenarios
...
.. index::
   pair: INDSCEN$ Instruction; Scenarios
...
.. index::
   pair: MAP$ Instruction; Scenarios
...
defines the source of the commarea. See the description of the :ref:`“TOVAR$ instruction” <#_V457UG_TOVAR$>` for further details of
...
.. _#_V457UG_MAP$_EVENTUAL_ELSETHEN:
...
:ref:`“Sending XML input as URL-encoded data” <#_V457UG_sending_XML>` for examples. To process requests containing XML data in plain
(unencoded) format, use instead the :ref:`“MAP$ FROM-INPUT” <#_V457UG_MAP$_FROM-INPUT>`.

.. _#_V457UG_MAP$_FROM-INPUT:
...
.. index::
   pair: OPTION$ Instruction; Scenarios
...
.. index::
   pair: Option IND$FILE Examples; Scenarios
...
.. index::
   pair: Option Examples; Scenarios
...
.. index::
   pair: PERFORM$ Instruction; Scenarios
...
This instruction terminates a subroutine called by a :ref:`“MAP$ EXECUTE” <#_V457UG_MAP$_EXECUTE>` instruction.
...
LINE” <#_V457UG_capability_types9 with the LINE parameter set to the line on which the scenario is running.
...
Sends an outbound message to a VIRTEL line. The name of the destination line is specified indirectly through a VIRTEL transaction. The SEND$ TO instruction must be preceded by an :ref:`“OPTION$” <#_V457UG_OPTION$>` instruction which sets up the parameters for the outbound message according to the line type.
...
Sends an outbound message on the specified line. SEND$ TO-LINE is identical to SEND$ TO except that the name of the line is specified directly in the scenario instead of via a transaction. The SEND$ TO-LINE instruction must be preceded by an :ref:`“OPTION$” <#_V457UG_OPTION$>` instruction which sets up the parameters for the outbound message according to the line type.
...
Sends the contents of the specified variable as an outbound message on a VIRTEL line. The name of the destination line is specified indirectly through a VIRTEL transaction. A SEND$ VARIABLE-TO instruction is equivalent to an :ref:`“OPTION$ instructions” <#_V457UG_OPTION$>`  instruction followed by a SEND$ TO instruction in the case where OPTION$ specifies only a variable name.
...
of via a transaction. A SEND$ VARIABLE-TO-LINE instruction is equivalent to an :ref:`“OPTION$” <#_V457UG_OPTION$>` instruction
...
country
    specifies the name of the ISO translation table to be used. Refer to the :ref:`“SET-COUNTRY-CODE” <#_V457UG_set-country-code>` for a list of valid country names. If country is not specified, the value of the COUNTRY parameter in the VIRTCT is used. 
...
See also the :ref:`“SET$ URL-ENCODING” <#_V457UG_SET$_URL-ENCODING>`.
...
.. _#_V457UG_SET$_SCREEN-POSITION:        
...
TO-NEXT-LINE
    sets the current position to the next line. If the next line is after the end of the current screen rectangle,the NOTFOUND condition will be raised. This condition can be tested by means of the :ref:`“IF$ instruction” <#_V457UG_IF$>`.
...
Refer to :ref:`“SET$ ENCODING” <#_V457UG_SET$_encoding>` for a description of the parameters.
...
can be used to generate XML. See :ref:`“VIRTEL as an XML parser/generator” <#_V457UG_XML_parser>`.
...
.. index::
   pair: SUGGEST$ Instruction; Scenarios
...
AJAX request containing the VerifyVirtelSession keyword (see :ref:`“3270 Session Management” <#_V457UG_session_management>`). By using an
...
VIRTEL as page templates (see :ref:`“Uploading HTML Pages” <#_V457UG_uploading_HTML_pages>`). The TYPE=LINEBUFFER parameters on the COPY$
...
.. index::
   triple: Sending Query Data with POST; Virtel; Scenarios
...
.. index::
   triple: Extracting Query Data; Virtel; Scenarios
...
.. index::
   pair: Query input Data; Transactions
...
.. _#_VIRT457UG_transaction_detail:
...
.. _#_V457UG_sending_XML:

.. index::
   pair: Encoding XML Data; XML
   pair: XML Data; Virtel
...
.. index::
     pair: Example of using XML Data; Scenarios
...
.. _#_V457UG_XML_parser:

.. index::
     pair: XML parser/generator; Virtel
...
.. index::
     triple: MAP$ Example; Scenarios; XML Parser/Generator
...
specified in the “TIOA at Logon” field causes the INITIAL scenario :ref:`“SCONVXML” <#_V457UG_sconvxml>` to be executed.
...
.. index::
     pair: Commarea conversion; XML Parser/Generator
     pair: Commarea conversion; XML
...
order specified in the “TIOA at Logon” field causes the INITIAL scenario :ref:`“SCONVPLA” <#_V457UG_sconvpla>` to be executed.
...
.. index::
     pair: Commarea conversion; XML Parser/Generator
     pair: Commarea conversion; Scenarios
...
.. index::
     triple: Automatic scenario generation; Scenarios; MAPGEN
...
FORMAT=format
    Specifies the type of scenario to be generated. The following formats may be specified:-
        ...
        QUERY
            Generates an INITIAL scenario which accepts input data encoded as HTML form-fields, and produces an output commarea. The generated scenario contains a :ref:`“MAP$ FROM-INPUT” <#_V457UG_MAP$_FROM-INPUT>`, QUERY instruction.
        XML
            Generates an INITIAL scenario which accepts XML input data in the body of an HTTP request, and produces an output commarea. The generated scenario contains a :ref:`“MAP$ FROM-INPUT” <#_V457UG_MAP$_FROM-INPUT>`, XML instruction.
        'XML=fieldname'
            Generates an INITIAL scenario which accepts XML input encoded within an HTML form-field named fieldname, and produces an output commarea. The generated scenario contains a :ref:`“MAP$ FROM-FIELD” <#_V457UG_MAP$_FROM-FIELD>` instruction with the parameter FIELD=fieldname.
        COM
            When VAR is not specified: Generates an INITIAL scenario which accepts an input commarea in the body of an HTTP request, and produces a set of VIRTEL variables whose names correspond to the fields in the copybook. The generated scenario contains a :ref:`“TOVAR$ FROM-INPUT” <#_V457UG_TOVAR$_FROM-INPUT>` instruction. 

            When VAR is specified: Generates an OUTPUT scenario which accepts an input commarea contained in the VIRTEL variable named by the VAR parameter, and produces a set of VIRTEL variables whose names correspond to the fields in the copybook. The generated scenario contains a :ref:`“MAP$ FROM-VARIABLE” <#_V457UG_MAP$_FROM-VARIABLE>` instruction.
        'COM=fieldname'
            Generates an INITIAL scenario which accepts an input commarea encoded within an HTML form-field named fieldname, and produces a set of VIRTEL variables whose names correspond to the fields in the copybook. The generated scenario contains a :ref:`“MAP$ FROM-FIELD” <#_V457UG_MAP$_FROM-FIELD>` instruction with the parameter FIELD=fieldname.
...
.. index::
   single: Incoming E-mails

.. _#_V457UG_email_incoming:
...
.. index::
     triple: File transfer; SMTP; Virtel
...
.. index::
     triple: Starting an application by e-mail; SMTP; Virtel
...
field to VIRTEL via the relay LU and reading the response (see :ref:`“Retrieve Data From An E-Mail” <#_V457UG_email_retrieve_data>`).
...
The first statement following the .GET command is the VIRTEL URL of the HTTP request (see :ref:`“Virtel URL formats” <#_V457UG_url_formats>`). Only the pathname, pagename, and tranname parts of the URL are required. The http://ipaddr:port part of the URL must be omitted.
...
The first statement following the .POST command is the VIRTEL URL of the HTTP request (see :ref:`“Virtel URL formats” <#_V457UG_url_formats>`). Only the pathname, pagename, and tranname parts of the URL are required. The http://ipaddr:port part of the URL must be omitted.
...
.. _#_V457UG_programming_interfaces:
...
.. index::
   pair: SRTVIRTEL; Programming Interfaces
...
template (see :ref:`“Handling table variables” <#_V457UG_table_variables>`), or by means of a scenario instruction (see “FIELD$ DEFINE-CHOICE”,
...
An application started by an incoming e-mail (see :ref:`“Incomming e-mails” <#_V457UG_email_incoming>`) can retrieve data from the content
...
.. index::
   ...
   triple: Encryption; AT-TLS;Virtel
...
.. index::
   ...
   triple: Certificates; AT-TLS;Virtel
...
WEB2AJAX.htm page template (see :ref:`“Presentation modes” <#_V457UG_presentation_modes>`).
...
.. index::
   pair: Using rules; Virtel Web Access Security
   pair: Using rules; Security
...
:ref:`“Correspondent Management” <#_V457UG_correspondent_management>`). The second rule, 1HT00200, assigns the entry point EPINTERN to clients
...
This form of a VIRTEL URL is described in the section :ref:`“Dynamic URL with user data” <#_V457UG_dynamic_URL>`.
...
which will normally be an application selection menu. For detailed information, see :ref:`“Virtel URL formats” <#_V457UG_url_formats>`.
...
.. index::
   pair: Access Host Application; HOWTOs 
...
process of connection to a host application, see :ref:`“Virtel URL formats” <#_V457UG_url_formats>` of this manual, and “Connection/
...
For more information see :ref:`“Virtel URL formats” <#_V457UG_url_formats>`.
...
The :ref:`“Web Access settings menu” <#_V457UG_settings_menu>` allows you to change the font family and font size. By default VIRTEL will
...
The :ref:`“Web Access settings menu” <#_V457UG_settings_menu>` allows you to change the display style to suit your needs. By default VIRTEL
...
The :ref:`“Web Access settings menu” <#_V457UG_settings_menu>` allows you to specify this configuration using the following settings:
...
The site default settings for Web Access are stored in the file w2hparm.js (see :ref:`“Global modification of Web Access settings” <#_V457UG_global_modifications>`)
