...
sessionid
    ...
    *{{{SESSION-CODE}}}* tag embedded in the template page (se`e “Session And Context Management” <#_V456UG_26 <#_V456UG_28>`__).
...
pagename
    Is the name of the template page to be used for displaying the print data. This page must contain the special *{{{PRINT}}}* tag (see “3287 Printing” <#_V456UG_61 <#_V456UG_64>`__) at the point at which the print data are to be inserted in the page.
...
capability
    ...
    generated by the `“{{{SET-HEADER}}}” <#_V456UG_60 <#_V456UG_61>`__ tag.
...
The security code, which identifies a user during page upload by HTTP, is normally transmitted between the browser and VIRTEL as a cookie (see `“Uploading HTML Pages” <#_V456UG_68 <#_V456UG_77>`__). If, for security or other reasons, your browser does not allow the use of cookies, then the security code can, as an alternative, be embedded in a URL of the form shown below:-
...
A URL embedded in a page sent by VIRTEL may contain a VirtelUserSignon parameter in order to propagate the user’s signon credentials to another page. The value of the parameter is generated by the *({{{USER-SIGNON-CODE}}}* tag, as described in `page 63 <#_V456UG_77>`__. This can be useful in cases where the browser does not propagate the user name and password from one page to another, for example when using VIRTEL transaction security type 4. The format of a URL containing a signon code is shown below:-
...
The SESSION-CODE tag allows a sessioncode to be inserted in the template page in the format VirtelSession=xxxxxxxxxxxxxxxx. The value of the sessioncode changes after each message. For examples of its use, see `“Transmitting Data To The Host” <#_V456UG_27 <#_V456UG_29>`__, and `“3287 Printing” <#_V456UG_61 <#_V456UG_64>`__.
...
BLANK-BINARY-ZEROES
    Affects the processing of the COPY-FROM and GENERATE-VARIABLES tags (see “Inserting host application data in a page” <#_V456UG_29).
...
HTML-ESCAPES, JAVASCRIPT-ESCAPES, JSON-ESCAPES, NO-ESCAPES, XML-ESCAPES
    Affects the processing of the CURRENT-VALUE-OF, TRIMMED-VALUE-OF and NO-BLANKS-VALUE-OF tags (see “Handling table variables” <#_V456UG_44). Similarly affects the processing of the values generated by GENERATEVARIABLES.
ID
...
A table variable is a list of values sent to VIRTEL by a host application in a structured field of type “FAE5 or FAE6”,^page 247. Table variables may also be created by means of a VIRTEL tag embedded in the page template (see :ref:`“CREATE-VARIABLE-FROM” <#_V456UG_create-variable-from>`), via a scenario (see :ref:`“COPY$ instructions” <#_V456UG_copy$>`), or by means of a S VARIABLE command contained in the host 3270 datastream (see “HOST4WEB commands” <#_V456UG_255).
...
xxxxxx
    ...
    CHARACTER-SET
        The name of the current UTF-8 character set, or the country code if the page is not in UTF-8 mode (see `“EBCDIC translation management” <#_V456UG_56`)__
    DATE-TIME
...
For more informations, see `“CREATE-VARIABLE-IF” <#_V456UG_52.`__
...
The USER-SIGNON-CODE tag generates a unique value called a signoncode which may be used with the VirtelUserSignon parameter of the URL to propagate the signed-on user’s credentials to another page (see `“Propagation of signon by URL” <#_V456UG_21 <bookmark14>`__).
...
The SECURITY-TOKEN and IF-SECURITY-TOKEN-IS-READY tags are used during uploading of HTML pages from a browser (see “Uploading pages by HTTP (secured by cookie)” <#_V456UG_69).
...
These files may then be passed to the VIRSVPDF service program which invokes MakePDF to generate output files in PDF format. Refer to “PDF output generation” <#_V456UG_213 for an example scenario.
...
- a correspondent may be authorized to upload HTML pages and other elements into an HTMLTRSF file. For further details, refer to “Uploading pages by SMTP” <#_V456UG_68 and “Uploading pages by HTTP (secured by cookie)” <#_V456UG_69.
...
1. Activate the e-mail correspondent: see “Account activation” under the heading “Correspondent Management” <#_V456UG_64. This triggers the sending of an e-mail containing the security code, as in the following example:-
...
1. (First time only) Activate the e-mail correspondent: see “Account activation” under the heading “Correspondent Management” <#_V456UG_64. This triggers the sending of an e-mail containing the security code, as in the following example:
...
uplbas
    external name of the upload transaction in VIRTEL which specifies the target directory (HTMLBAS). See “Uploading pages by HTTP (secured by signon)” <#_V456UG_75 for a list of upload transactions.
...
VIRTEL Web Access which are stored in the w2hparm.js file. See “Global modification of Web Access settings” <#_V456UG_99.
...
To add or remove a style, see “Customize display style” <#_V456UG_280)

Administrators can add some new customs style as describe in the section “How to customize display style” <#_V456UG_280
...
Administrators can add some new customs print style as describe in the section “How to customize print style” <#_V456UG_281
...
You can also use custom.js to remove icons individually from the toolbar, see “Removing unwanted toolbar icons” <#_V456UG_126.
...
To hide the toolbar completely, see “Hiding the toolbar” <#_V456UG_121.

To hide only the Virtel Application name, see “Showing / Hiding server informations” <#_V456UG_121.
...
Modernisation VIRTEL Scenarios” <#_V456UG_147.
...
The custom.js file must be activated as described in “Site customization of Javascript functions” <#_V456UG_124.
...
useful (refer to “IF$ instructions” <#_V456UG_177 for further details).
...
MAXTIME=nnn
     (optional) specifies the maximum time, in hundredths of a second, that the scenario will wait for a message from the application. When this time expires, the APPLICATION-TIMEOUT flag will be set (see “IF$” <#_V456UG_177 instruction) and the scenario will resume execution. If the MAXTIME parameter is not specified, then the scenario will wait indefinitely, or until VIRTEL disconnects the session when the timeout value of the entry point is exceeded.
LASTMSG=YES
...
CURRENT-SCREEN-POSITION
    The tested variable is located at the current screen position as set or incremented by “SET$ SCREEN-POSITION” <#_V456UG_201.
LTRIM
...
.. note::
    Note 1: For row, col and len, a value of '=' means the value of the current rown col or len as set by “SET$ SCREENPOSITION” <#_V456UG_201
...
If the input request does not contain the requested file, the NOT-FOUND condition will be raised. This condition can be tested by means of the “IF$ instruction” <#_V456UG_177.
...
name1
    the name of an HTTP query parameter (see “VIRTEL URL formats” <#_V456UG_11)
row,col,len
...
If the parameter name1 is not present in the input request, the NOT-FOUND condition will be raised. This condition can be tested by means of the “IF$” <#_V456UG_177 instruction.
...
type
    (optional) may indicate one of the following values:
        ...
        TYPE=REPLACE
            indicates that the new value will replace the existing value of the variable. If TYPE=REPLACE is not specified, and the variable already exists, the new value will be appended to any existing values. If the parameter name1 is not present in the input request, the NOT-FOUND condition will be raised. This condition can be tested by means of the “IF$” <#_V456UG_177 instruction.
...
type
    (optional) indicates that the result variable has a special format. The following values are possible:
        ...
        TYPE=LINEBUFFER
            indicates that the result should be converted to MakePDF “LINEBUF” format for input to the VIRSVPDF service program. The page template contains special tags (see “Generating PDF output” <#_V456UG_62) which are converted to the format required by MakePDF. 

            If the specified file does not exist in the current VIRTEL directory, the NOT-FOUND condition will be raised. This condition can be tested by means of the “IF$” <#_V456UG_177 instruction.
...
.. note::      
    ...
    Note 1: for row, col and len, a value of '=' means the value of the current row, col or len as set by “SET$ SCREENPOSITION” <#_V456UG_201
...
NEW-PASSTICKET-FOR-TRANSACTION
    Generate a PassTicket allowing the current USERID. If the generation fails, the NOT-FOUND condition will be raised. This condition can be tested by means of the “IF$” <#_V456UG_177 instruction.
...
item
    When class is NAME-OF or VALUE-OF, the following VIRTEL data items may be specified:
        CHARACTER-SET
            The name of the current UTF-8 character set, or the country code if the page is not in UTF-8 mode (see “EBCDIC translation management” <#_V456UG_56)
        CURRENT-FOREACH-INDEX
            Value of the current index in a FOREACH loop.
        CURRENT-FOREACH-VARIABLE
            Value of the variable at current index in a FOREACH loop (see “FOREACH$ VALUE-IN-VARIABLE” <#_V456UG_176).
        ...
        CURRENT-HEIGHT
            The current values of the named field in the “SET$ SCREEN-POSITION” <#_V456UG_201.
        DATE-TIME
            The current date and time (14 characters in the format YYYYMMDDHHMMSS)
        DIRECTORY
            The current VIRTEL directory name
...
row,col,len
    row, column, and length of the destination field on the screen. If the SCREEN parameter is not specified, the variable will be copied to the current cursor position. For row, col and len, a value of '=' means the value of the current row, col or len as set by “SET$ SCREEN-POSITION” <#_V456UG_201.
type
    (optional) may indicate one of the following values:

        TYPE=ERASE-FIELD
            indicates that the destination field is cleared to nulls before copying the data. If the variable name2 does not exist, the NOT-FOUND condition will be raised. This condition can be tested by means of the “IF$” <#_V456UG_177 instruction. If the destination row and column specify a protected field of the 3270 screen, the scenario terminates abnormally and message VIRS129E is issued to the system console. If the destination field is unprotected but the variable value is too long for the field, the data will be silently truncated.
...
class
    identifies the type of VIRTEL system field to be updated. The following values may be specified:
        ...
        OLD-PASSTICKET-FOR-TRANSACTION
            Evaluates the Passticket currently in 'varname' for the transaction 'item'. If the evaluation fails, the NOT-FOUND condition will be raised. This condition can be tested by means of the “IF$” <#_V456UG_177 instruction.
item
    When class is NAME-OF or VALUE-OF, the following VIRTEL data items may be specified:
        ...
        ROUTING-PARAMETER
            Allows an Identification Scenario to override the value of the routing parameter specified in the userdata field of the URL (see “Virtel URL formats” <#_V456UG_11).
...
TRACE,SCENARIO or NOTRACE,SCENARIO
    Starts or stops a trace on the scenario. The trace is downloadable by means of the “CREATE-VARIABLE-IF” <#_V456UG_52 tag intended for use by Virtel Studio.
SNAP
...
defines the source of the commarea. See the description of the “TOVAR$ instruction” <#_V456UG_203 for further details of
...
This instruction terminates a subroutine called by a :ref:`“MAP$ EXECUTE” <#_V456UG_186>` instruction.
...
LINE” <#_V456UG_199 with the LINE parameter set to the line on which the scenario is running.
...
country
    specifies the name of the ISO translation table to be used. Refer to the “SET-COUNTRY-CODE” <#_V456UG_56 for a list of valid country names. If country is not specified, the value of the COUNTRY parameter in the VIRTCT is used. 
...
See also the “SET$ URL-ENCODING” <#_V456UG_202.
...
TO-NEXT-LINE
    sets the current position to the next line. If the next line is after the end of the current screen rectangle,the NOTFOUND condition will be raised. This condition can be tested by means of the “IF$” <#_V456UG_177 instruction.
...
Refer to “SET$ ENCODING” <#_V456UG_199 for a description of the parameters.
...
can be used to generate XML. See “VIRTEL as an XML parser/generator” <#_V456UG_223.
...
AJAX request containing the VerifyVirtelSession keyword (see “3270 Session Management” <#_V456UG_17). By using an
...
VIRTEL as page templates (see “Uploading HTML Pages” <#_V456UG_68). The TYPE=LINEBUFFER parameters on the COPY$
...
.. note::
    ...
    It is also possible for the application program to create VIRTEL table variables directly, either by means of an “FAE5” <#_V456UG_247 structured field, or via the HOST4WEB command “SET VARIABLE” <#_V456UG_0. The FAE5 structured field allows the application to pass large amounts of raw data to VIRTEL without being constrained by the limitations of the 3270 screen geometry. For such “VIRTEL-aware” applications, an output scenario is not necessary.
...
specified in the “TIOA at Logon” field causes the INITIAL scenario “SCONVXML” <#_V456UG_0 to be executed.
...
order specified in the “TIOA at Logon” field causes the INITIAL scenario “SCONVPLA” <#_V456UG_0 to be executed.
...
FORMAT=format
    Specifies the type of scenario to be generated. The following formats may be specified:-
        ...
        COM
            When VAR is not specified: Generates an INITIAL scenario which accepts an input commarea in the body of an HTTP request, and produces a set of VIRTEL variables whose names correspond to the fields in the copybook. The generated scenario contains a “TOVAR$ FROM-INPUT” <#_V456UG_203 instruction. 
...
XMLGEN=YES|NO
    Specifies whether an additional XMLGEN step is to be run. This parameter is useful when FORMAT=COM or FORMAT='COM=fieldname' is specified. If XMLGEN=YES then the job also generates a VIRTEL page template suitable for converting the VIRTEL result variables into XML format. The generated page template is written to userid.XMLGEN.OUTPUT which must be preallocated as a sequential dataset. The contents of this dataset should then be transferred to your workstation where it can be uploaded into a VIRTEL directory (see “Uploading pages by HTTP” <#_V456UG_68).
...
Only authorized users (see “Correspondent Management” <#_V456UG_64) can perform transfers.
...
field to VIRTEL via the relay LU and reading the response (see “Retrieve Data From An E-Mail” <#_V456UG_258).
...
The first statement following the .GET command is the VIRTEL URL of the HTTP request (see “Virtel URL formats”,
page 11). Only the pathname, pagename, and tranname parts of the URL are required. The http://ipaddr:port part of
the URL must be omitted.
...
The first statement following the .POST command is the VIRTEL URL of the HTTP request (see “Virtel URL formats”,
page 11). Only the pathname, pagename, and tranname parts of the URL are required. The http://ipaddr:port part of
the URL must be omitted.
...
template (see “Handling table variables” <#_V456UG_44), or by means of a scenario instruction (see “FIELD$ DEFINE-CHOICE”,
...
An application started by an incoming e-mail (see “Incomming e-mails” <#_V456UG_229) can retrieve data from the content
...
WEB2AJAX.htm page template (see “Presentation modes” <#_V456UG_103).
...
“Correspondent Management” <#_V456UG_64). The second rule, 1HT00200, assigns the entry point EPINTERN to clients
...
This form of a VIRTEL URL is described in the section “Dynamic URL with user data” <#_V456UG_0.
...
which will normally be an application selection menu. For detailed information, see “Virtel URL formats” <#_V456UG_11.
...
process of connection to a host application, see “Virtel URL formats” <#_V456UG_11 of this manual, and “Connection/
...
For more information see “Virtel URL formats” <#_V456UG_11.

5.5. How To Change The Font For Web Access
------------------------------------------

The “Web Access settings menu” <#_V456UG_0 allows you to change the font family and font size. By default VIRTEL will
...
The “Web Access settings menu” <#_V456UG_0 allows you to change the display style to suit your needs. By default VIRTEL
...
The “Web Access settings menu” <#_V456UG_0 allows you to specify this configuration using the following settings:
...
The site default settings for Web Access are stored in the file w2hparm.js (see “Global modification of Web Access settings” <#_V456UG_99)

5.10. How To Change The User Settings Interface
-----------------------------------------------

The user settings interface can be changed using the settingsGUI parameter stored in file w2hparm.js (see “Global modification of Web Access settings” <#_V456UG_99)
