...
- Enhanced support for Japanese codepages: improved support for IBM1390 codepage, and new options to switch codepage without disconnecting the session. New option to select JIS display mode (JIS78 / JIS83 / JIS90 / JIS97).
...
Cleanup Correspondents and Security admin panels and applications.
...
**DDI Directories are empty when CRYPTS parameter is specified**

The CRYPTS parameter was causing an error in DDI processing. This is fixed.

**Disable default Debug Option (DOPTIONS) for LU Nailing**
...
A new message in the log when Virtel detects a connection attempt with an invalid or missing session cookie:
