...
1. Copy the VTAM application definition created by ARBOLOAD to
   ...
   the following z/OS command:-
    ...
    RHTIP??? APPL
    AUTH=(ACQ,PASS),MODETAB=ISTINCLM,DLOGMOD=DSILGMOD,EAS=1

1. Before running VIRTEL as a batch job modify the program name to
...
1. Tracing an application. The ARBOLOAD sets up defaults for VIRTEL. One
    ...
    **F SPTHOLTA,LINES **
    ...
    I can see that HTTP-W2H is the external name of a line and is
    ...
    following url is entered:-

    http://192.168.170.30:41001/w2h/WEB2AJAX.htm+TSO

    This will display a TSO session on the web browser. The +TSO
    ...
    trace on the correct terminal/relay.

    Displaying the VITREL relays indicates which session have been
    establish.
...
This shows a TSO session on Terminal/Relay DEVTA015/REHVT001

Using this information, I can start a trace on that terminal/relay.
...
+VIR0062I DEVTA015 TRACE ACTIVE
    ...
    Now the TSO session can continue with and any 3270 data stream data
    will be written to the VIRTRACE dataset.
