...
4. IDP returns selected attributes to SP. In this case only the UID is
   required.

5. SP redirects user to Virtel APPMENU passing the UID as a header in
...
7. User selects applications and logs on without specifying userid or
...
In this file we identify the LDAP server the IDP will use. We also set the TLS security flag and identify the Java trustStore. ::
...
SP Configuration maintained on Windows workstation HOLT-WIN7
